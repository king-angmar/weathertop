package xyz.wongs.weathertop.shiro.filter;
	...
	protected boolean onAccessDenied(ServletRequest request,
		...
		if (!subject.isAuthenticated() && !subject.isRemembered()) {
			...
			if (ShiroFilterUtils.isAjax(request) ) {
				...
				responseResult.setCode(ResponseCode.MANY_LOGINS.getCode());
				responseResult.setMsg(ResponseCode.MANY_LOGINS.getMsg());
		...
		try {
			// 当前用户
//			SAccount account = (SAccount) subject.getPrincipal();
//			String userName = account.getAccountname();
	...
	public static void out(ServletResponse response, ResponseResult result){
		...
		try {
			...
			log.error("用户在线数量限制】响应json信息出错", e);
		}finally{
			if(null != out){
				out.flush();
				out.close();
	...
	private boolean isAjaxResponse(ServletRequest request,
		...
		if (ShiroFilterUtils.isAjax(request) ) {
			log.debug(getClass().getName()+ "当前用户已经在其他地方登录，并且是Ajax请求！");
			responseResult.setCode(ResponseCode.MANY_LOGINS.getCode());
			responseResult.setMsg("您已在别处登录，请您修改密码或重新登录");
			out(response, responseResult);
		}else{
			// 重定向
			WebUtils.issueRedirect(request, response, kickoutUrl);
