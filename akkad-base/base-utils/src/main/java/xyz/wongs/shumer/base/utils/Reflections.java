 ...
 */
package xyz.wongs.shumer.base.utils;
			...
			for(Field targField:targetFields){
				...
				for(Field sourceField:sourceFields){
					...
					if(targFieldName.equals(sourceFieldName) && targFieldType.equals(sourceFieldType)){
						String fieldGetName = xyz.wongs.shumer.base.utils.StringUtils.parGetName(sourceField.getName());
						if (!xyz.wongs.shumer.base.utils.StringUtils.checkGetMet(sourceMethods, fieldGetName)) {
							continue;
						}
						Method sourceFieldGetMet = sourceClz.getMethod(fieldGetName);
						Object value = sourceFieldGetMet.invoke(source);
						if(null==value || xyz.wongs.shumer.base.utils.StringUtils.isBlank(value.toString())){
							continue;
						}
						Object val = getValue(value.toString(),targFieldType);
						//Set Method
						String fieldSetName = xyz.wongs.shumer.base.utils.StringUtils.parSetName(targField.getName());
						if (!xyz.wongs.shumer.base.utils.StringUtils.checkGetMet(targetMethods, fieldSetName)) {
							continue;
						}
	...
	public static int getFieldValueMap(String[] s,Object bean,Boolean isCheck){
		...
		for (int i=0;i<s.length;i++) {
			...
			try {
				...
				String fieldGetName = xyz.wongs.shumer.base.utils.StringUtils.parGetName(fields[i].getName());
				//校验是否有GETTER、SETTER的方法
				if (!xyz.wongs.shumer.base.utils.StringUtils.checkGetMet(methods, fieldGetName)) {
					continue;
				}
				//Type conversion
